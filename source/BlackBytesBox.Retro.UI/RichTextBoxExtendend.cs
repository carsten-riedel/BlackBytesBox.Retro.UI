...
using BlackBytesBox.Retro.UI.Extensions;

namespace BlackBytesBox.Retro.UI
{
    /// <summary>
    /// RichTextBox with extended features: output-only display preserving background, optional caret hiding, and utility scrolling.
    /// </summary>
    public class RichTextBoxExtendend5 : RichTextBox
    {
    ...
    /// RichTextBox with extended features: output-only display preserving background, optional caret hiding, and utility scrolling.
    /// </summary>
    public class RichTextBoxExtendend4 : RichTextBox
    {
        // Win32 API to hide the text caret
        [DllImport("user32.dll")]
        private static extern bool HideCaret(IntPtr hWnd);
        ...
        public void AppendTextLine(string text)
        {
            ...
            // Append buffered lines without changing scroll
            base.Text = base.Text + text + Environment.NewLine;
            //base.AppendText(text + Environment.NewLine);
        ...
        /// </summary>
        /// <remarks>
        /// When false, any attempt—mouse or keyboard—to select text is blocked outright.
        /// </remarks>
        [DefaultValue(false)]
        [Category("Extended Features")]
        [Description("When false, selection is fully disabled.")]
        ...
        protected override CreateParams CreateParams
        {
            get
            {
                ...
                // make control non‐selectable at the WinForms level if desired
                if (!AllowSelection)
                {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            // block any selection message or mouse-down that could start a selection
            if (!AllowSelection &&
                (m.Msg == EM_SETSEL
        ...
        {
            // block Shift+arrows, Ctrl+A, etc.
            if (!AllowSelection &&
                (keyData.HasFlag(Keys.Shift)
              || (keyData & (Keys.Control | Keys.A)) == (Keys.Control | Keys.A)))
            {
                return true; // handled
            }
        ...
        protected override void OnGotFocus(EventArgs e)
        {
            base.OnGotFocus(e);
            if (!AllowSelection)
                Parent?.Select();  // avoid caret or selection
        }
    }
